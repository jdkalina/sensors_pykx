...
def get_runs(tool_ids):
    ...
    r= db.ees_run.select(where = kx.Column('tool_id').isin(tool_ids))
    r = kx.q.xkey('tool_id', r)
    t= db.ees_tool_lookup.select(columns=['tool_id','tool_name'],where = kx.Column('tool_id').isin(tool_ids))
    t = kx.q.xkey('tool_id', t)
    return kx.q.lj(r,t)

if __name__ == '__main__':
    tool_id = kx.toq(5001, ktype=kx.IntAtom)
    get_runs(tool_ids =tool_id)
