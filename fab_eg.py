...
sd = db.ees_sensor_data.select(columns=['tool_id','time_stamps','data_float','data_long','data_str','sensor_id','ts_id'], 
							   where=[kx.Column('min_time')> r['start_time'].min(), kx.Column('max_time')<= r['end_time'].max(), kx.Column('tool_id').isin(r['tool_id']), kx.Column('min_time').within(minTime, maxTime)])
