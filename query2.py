...
def main():
    ...
    for i in dates:
        ...
        if r.shape[0] == 0:
            ...
            sd = db.ees_sensor_data.select(columns=['tool_id','time_stamps','ts_id', 'data_float'], where=[kx.Column('date')==i, kx.Column('ts_id').isin(ep['ts_id']), kx.Column('min_time') >= start_time, kx.Column('max_time')<= end_time])
