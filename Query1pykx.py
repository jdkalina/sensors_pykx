...
kx.q('select from ees_tool_lookup where proc_type_id = 6990')
...
sinfo = db.ees_sensor_lookup.select(columns = [kx.Column('id').name('sensor_id'),
                                                kx.Column('name'),
...
cinfo = db.ees_sensor_lookup.select(columns = [kx.Column('proc_type_id'),kx.Column("id"),kx.Column("name").name('recipe')],
                                    where=[
                                        kx.Column('proc_type_id').isin(tinfo['proc_type_id']),
                                        kx.Column('name').like('Recipe')
                                    ])

runctx = db.ees_run_context.select(columns=['tool_id','run_id','start_time','ctx_value'],
                                   where=[
                                       kx.Column('date').within(stdt,enddt),
...
sd = db.ees_sensor_data.select(columns=['tool_id','sensor_id','ts_id','time_stamps','data_float'],
                          where=[
                              kx.Column('date').within(starttime, endtime),
                              kx.Column('ts_id').isin(ts['ts_id']),
                              kx.Column('min_time') >= runctx['start_time'].min()
                          ]
                          )
