...
def main():
  ...
  sinfo = db.ees_sensor_lookup.select(columns = [kx.Column('id').name('sensor_id'),kx.Column('name'),kx.Column('proc_type_id')],where = [kx.Column('proc_type_id').isin(tinfo['proc_type_id']),kx.Column("name").like("*Pressure*"), kx.Column("name").like("IG*")]) # returns back sensor_id 2394585
  ...
  cinfo = db.ees_sensor_lookup.select(columns = [kx.Column('proc_type_id'),kx.Column("id"),kx.Column("name").name('recipe')],where=[kx.Column('proc_type_id').isin(tinfo['proc_type_id']),kx.Column('name').lower().like('*recipe*')]  
